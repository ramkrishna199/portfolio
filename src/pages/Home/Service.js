import React from "react";

const Service = () => {
  return (
    <>
      <div className="container-fluid bg-light my-5 py-6" id="service">
        <div className="container">
          <div className="row g-5 mb-5 wow fadeInUp" data-wow-delay="0.1s">
            <div className="col-lg-6">
              <h1 className="display-5 mb-0">Internship</h1>
            </div>
          </div>
          <div className="row g-4">
            <div className="col-lg-6 wow fadeInUp" data-wow-delay="0.3s">
              <div className="service-item d-flex flex-column flex-sm-row bg-white rounded h-100 p-4 p-lg-5">
                <div className="bg-icon flex-shrink-0 mb-3">
                  <i className="fa fa-laptop-code fa-2x text-dark" />
                </div>
                <div className="ms-sm-4">
                  <h4 className="mb-3">AI Analysis Intern</h4>
                  <h6 className="mb-3">
                    From{" "}
                    <span className="text-primary">
                      Federal Synergies India Pvt.Ltd
                    </span>
                  </h6>
                  <span>
                    Technical Stack Lernt : Anaconda, LabelMe, Python, Data
                    segration,Data anotation
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div className="container-fluid bg-light my-5 py-6" id="service">
        <div className="container">
          <div className="row g-5 mb-5 wow fadeInUp" data-wow-delay="0.1s">
            <div className="col-lg-6">
              <h1 className="display-5 mb-0">Certifications</h1>
            </div>
            <div className="col-lg-6 text-lg-end">
              <a className="btn btn-primary py-3 px-5" href>
                Hire Me
              </a>
            </div>
          </div>
          <div className="row g-4">
            <div className="col-lg-6 wow fadeInUp" data-wow-delay="0.1s">
              <div className="service-item d-flex flex-column flex-sm-row bg-white rounded h-100 p-4 p-lg-5">
                <div className="bg-icon flex-shrink-0 mb-3">
                  <i className="fa fa-crop-alt fa-2x text-dark" />
                </div>
                <div className="ms-sm-4">
                  <h4 className="mb-3">Full Stack Web Developer</h4>
                  <h6 className="mb-3">
                    From <span className="text-primary">Newton School</span>
                  </h6>
                  <span>
                    Technical Stack Lernt : Html, CSS, Javascript, React , Node,
                    MongoDB
                  </span>
                </div>
              </div>
            </div>
            <div className="col-lg-6 wow fadeInUp" data-wow-delay="0.3s">
              <div className="service-item d-flex flex-column flex-sm-row bg-white rounded h-100 p-4 p-lg-5">
                <div className="bg-icon flex-shrink-0 mb-3">
                  <i className="fa fa-code-branch fa-2x text-dark" />
                </div>
                <div className="ms-sm-4">
                  <h4 className="mb-3">
                    Full Stack Web Development Using Python on Django
                  </h4>
                  <h6 className="mb-3">
                    From{" "}
                    <span className="text-primary">
                      Mysirg Education Services Pvt.Ltd.
                    </span>
                  </h6>
                  <span>
                    Technical Stack Lernt : Html, CSS, Javascript, Django
                  </span>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
};

export default Service;
